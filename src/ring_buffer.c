...
ssize_t ring_buffer_enqueue(so_ring_buffer_t *ring, void *data, size_t size)
{
	...
	if (ring->cap - ring->write_pos >= size) {
		...
		size_t first = ring->cap - ring->write_pos;
		size_t bytes_left = size - first;

		memcpy(&ring->data[ring->write_pos], data, first);
		memcpy(&ring->data[0], (char *)data + first, bytes_left);
			    ...
			    unsigned long *seq)
{
	...
	if (ring->cap - ring->read_pos >= size) {
		...
		size_t first = ring->cap - ring->read_pos;
		size_t bytes_left = size - first;

		memcpy(data, &ring->data[ring->read_pos], first);
		memcpy((char *)data + first, &ring->data[0], bytes_left);
